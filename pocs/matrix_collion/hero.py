...
class Hero(PhysicalSpriteObject):
    ''' Hero Sprite Class '''
    def __init__(self, update_health, update_potions_count, batch2, group2, start_pos=(20, 200), hero_images=resources.HeroImages(), *args, **kwargs):
        super().__init__(img=hero_images.face_down, x=start_pos[0], y=start_pos[1], batch2=batch2, group2=group2, *args, **kwargs)
    ...
    def on_key_release(self, symbol, modifiers):
        if symbol == key.UP:
            ...
            self.change_health_amount(-1)
        elif symbol == key.P:
            if self.healing_potion_count >= 1 and self.health < self.max_health:  
                self.change_health_amount(1)
